...
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/manager.conf :mkdirp yes
:END:
...
*** Optical module
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/optical_module.geom
:END:
**** Preamble
#+BEGIN_SRC shell
...
*** Source
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/source.geom
:END:
**** Preamble
#+BEGIN_SRC shell
  #@description List of geometry models
...
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/lab_world.geom
:END:
**** Preamble
#+BEGIN_SRC shell
  #@description List of geometry models
...
#+BEGIN_SRC shell
  ...
  internal_item.labels : string[3] = "source" "detector" "box" "absorber0"
  ...
  internal_item.placement.source    : string  = "0 0 0 (cm) / y +90 (degree)"
  ...
  #@description The mapping directives for the "absorber" daughter volume
  mapping.daughter_id.absorber0 : string  = "[absorber.gc]"
#+END_SRC
**** World volume
#+BEGIN_SRC shell
...
** Mapping categories
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/categories.lis
:END:
#+BEGIN_SRC shell
  ...
  #######################################
  [category="absorber.gc" type="2034"]

  #@config The geometry category of the absorber

  #@description The GID address of a light guide inherits the GID address of the optical module
  inherits : string[1] = "lab.gc"
#+END_SRC
** Materials plugin
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/materials_plugin.conf
:END:
...
*** COMMENT Materials
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/materials.def
:END:
**** Insert preamble
#+BEGIN_SRC shell
...
*** Material aliases
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/materials_aliases.def
:END:
#+BEGIN_SRC shell
...
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/magnetic_field_plugin.conf
:END:
*** Plugin declaration
#+BEGIN_SRC shell
...
*** Magnetic fields
:PROPERTIES:
:HEADER-ARGS: :tangle config/geometry/magnetic_field_drivers.conf
:END:
**** Preamble
#+BEGIN_SRC shell
...
:PROPERTIES:
:HEADER-ARGS: :mkdirp yes :tangle config/visualization/geant4_validation.sty
:END:
*** Browser settings
#+BEGIN_SRC shell
...
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/g4_manager.conf
:END:
*** Manager setup
#+BEGIN_SRC shell
  [name="manager"]
...
#+BEGIN_SRC shell
  ...
  sensitive.scin_SD.record_alpha_quenching          : boolean = true

  #@description Flag to activate the recording of any track ID
  sensitive.scin_SD.record_track_id                 : boolean = true

  #@description Flag to activate the recording of any primary particle flag
  sensitive.scin_SD.record_primary_particle         : boolean = true

  #@description Flag to drop hits with zero energy deposit
  sensitive.scin_SD.drop_zero_energy_deposit_steps  : boolean = false

  #@description Flag to track gamma particles
  sensitive.scin_SD.track_gamma                     : boolean = true
#+END_SRC
...
#+BEGIN_SRC shell
  ...
  using_magnetic_field              : boolean = false
...
:PROPERTIES:
:HEADER-ARGS: :tangle   config/pipeline/particles.conf
:END:
...
***** EM physics process
:PROPERTIES:
:HEADER-ARGS: :tangle   config/pipeline/em_processes.conf
:END:
****** General settings
#+BEGIN_SRC shell
...
** Genbb configuration
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/genbb_manager.conf
:END:
...
:PROPERTIES:
:HEADER-ARGS: :tangle   config/pipeline/calibrations.conf
:END:
**** Insert preamble
#+BEGIN_SRC shell
...
*** Guns configuration
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/guns.conf
:END:
**** Insert preamble
#+BEGIN_SRC shell
  #@description A sample list of setups
...
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/genvertex_manager.conf
:END:
#+BEGIN_SRC shell
...
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/vgs.conf
:END:
**** Insert preamble
#+BEGIN_SRC shell
  #@description A sample list of setups
...
#+BEGIN_SRC shell
  ...
  mode.surface.side   : boolean = false
...
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/step_hit_processor_factory.conf
:END:
*** Insert preamble
#+BEGIN_SRC shell
...
*** Absorber hits
#+BEGIN_SRC shell
  [name="absorber.hit_processor" type="mctools::calorimeter_step_hit_processor"]

  #@config The step hit processor for the absorber
  ...
  mapping.category      : string = "absorber.gc"

  #@description The name of the geometry category to be attached to the output MC hits
  ##mapping.category.any_addresses :  string[1] = "part"
#+END_SRC
#+BEGIN_SRC shell
  [name="absorber.visu.hit_processor" type="mctools::push_all_step_hit_processor"]
...
to the other, can be set when executing the =bxdpp_processing= (see Section [[Use
and execute =geant4_validation= module]]).

** Module manager
:PROPERTIES:
:HEADER-ARGS: :mkdirp yes :tangle config/pipeline/module_manager.conf
:END:
...
** Service manager
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/service_manager.conf
:END:
*** Logging priority
#+BEGIN_SRC shell
...
:PROPERTIES:
:HEADER-ARGS: :tangle no :results output
:END:
...
*** Libraries
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/dlls.conf :results none
:END:
...
** Simulation module
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/geant4_simulation_module.conf
:END:
...
#+BEGIN_SRC shell
  ...
  manager.event_generator_name  : string  = "Am241"
#+END_SRC
*** Simulation manager
#+BEGIN_SRC shell
...
** Validation module
:PROPERTIES:
:HEADER-ARGS: :tangle   config/pipeline/geant4_validation_module.conf
:END:
...
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/histogram_templates.conf
:END:
...
** Chain module
:PROPERTIES:
:HEADER-ARGS: :tangle config/pipeline/geant4_chain_module.conf
:END:
...
:HEADER-ARGS: :tangle config/pipeline/services.conf
:END:
...
#+BEGIN_SRC shell
  bxdpp_processing                                                     \
      --dlls-config $PWD/config/pipeline/dlls.conf                     \
      --module-manager-config $PWD/config/pipeline/module_manager.conf \
      --datatools::resource-path="geant4_validation_dir@$PWD"          \
      --module geant4_chain_module                                     \
      --max-records 100                                                \
      --output-file /tmp/${USER}/bayeux.d/io_ouput_geant4_simulation.brio
#+END_SRC

The line with =--datatools::resource-path= tells the program where the
configuration file are stored /i.e./ =$PWD/config= in this case.
...
If you have installed the =Falaise= suite, you can use the =flvisualize= binary to
visualize the result of the simulation. The next command launches the
=flvisualize= with the dedicated style file defined in Section [[Visualization
settings]].

#+BEGIN_SRC shell
  flvisualize                                                             \
      --datatools::resource-path="geant4_validation_dir@$PWD"             \
      --detector-config-file $PWD/config/geometry/manager.conf            \
      --style-config-file $PWD/config/visualization/geant4_validation.sty \
      --input-file /tmp/${USER}/bayeux.d/io_ouput_geant4_simulation.brio
#+END_SRC
* Footnotes
